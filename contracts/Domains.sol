...
contract Domains is ERC721URIStorage {
    ...
    mapping(uint256 => string) public names; // Record domains registered

    //errors messaging saving gas
    error Unauthorized();
    error AlreadyRegistered();
    error InvalidName(string name);

    // We make the contract "payable" by adding this to the constructor
    constructor(string memory _tld) payable ERC721("tokio", "tokio") {
    ...
    function register(string calldata name) public payable {
        // if the name domains is already register revert the operation sending the error.
        if (domains[name] != address(0)) revert AlreadyRegistered();
        //check if the domain name is valid if not revert the operation
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Hashira name service", "image": "data:image/svg+xml;base64,',
        ...
        names[newRecordId] = name; // register the domains name to the mapping to get it after.
        _tokenIds.increment();
    }
    ...
    function setRecord(string calldata name, string calldata record) public {
        // Check that the owner is the owner if the domain name
    ...
    // withdraw balance of the contract
    ...
    //Valid if the name is not too small o too big.
